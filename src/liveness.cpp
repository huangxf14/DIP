...
vector<Point2i> GetStandardKeypoints(int w, int h, int stage) {
  ...
  init_keypoints[1].push_back(Point2i(152, 300) * ratio + delta); // finger 1
  ...
  init_keypoints[2].push_back(Point2i(152, 400) * ratio + delta); // finger 1
...
void LivenessDetector::Detect() {
  ...
  while (true) {
    ...
    vector<Point2i> match;
    frame.MatchKeypoints(keypoints, match, 20);
